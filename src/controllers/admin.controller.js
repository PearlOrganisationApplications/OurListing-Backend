...
export const searchMortgages = async (req, res) => {
  try {
    const {
      propertyType,
      propertyAddress,
    ...
    // 1. Text Search (Partial match for Address)
    if (propertyAddress) {
    ...
    if (buyerIntent) query.buyerIntent = buyerIntent;

    // 3. Range Filters (Price, Loan, FICO)
    ...
    if (minLoan || maxLoan) {
      query.requestedLoan = {};
      if (minLoan) query.requestedLoan.$gte = Number(minLoan);
      if (maxLoan) query.requestedLoan.$lte = Number(maxLoan);
    }

    if (minFico || maxFico) {
      query.ficoScore = {};
      if (minFico) query.ficoScore.$gte = Number(minFico);
      if (maxFico) query.ficoScore.$lte = Number(maxFico);
    }

    // 4. Execute Query with Pagination
    const skip = (page - 1) * limit;
    
    const results = await MortgageListing.find(query)
